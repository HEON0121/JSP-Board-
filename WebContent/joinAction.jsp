...
<body>
	...
	if(user.getUserID() == "" || user.getUserPassword() == "" || user.getUserName() == "" || user.getUserGender() == "" || user.getUserEmail() == "") {
			PrintWriter script = response.getWriter();
			script.println("<script>");
			script.println("alert('입력사항에 모두 입력을 해주세요.')");
			...
			if(result == -1){
				...
				session.setAttribute("userID", user.getUserID()); // 세션 아이디 속성 부여
				PrintWriter script = response.getWriter();
				script.println("<script>");
